#!/usr/bin/env python3
"""Converted from /var/nfs_share/Overfitting/CollegeExperience/Shift_Analysis/Label_Shift /Stress.ipynb for dataset CollegeExperience."""
...
# Load the dataset /var/nfs_share/Overfitting/CollegeExperience/data_preprocessing/college_experience_dataset/processed_data/android/android_2week_aggregations.csv
df = pd.read_csv(os.path.join(str(DATA_ROOT / "data_preprocessing/college_experience_dataset/processed_data/android/android_ios_2week_aggregations_normalized_per_feature.csv")))
...
label_counts = df_selected.groupby(['uid', 'stress_binary_personal']).size().reset_index(name='count')

# Pivot the data to have labels as columns
label_pivot = label_counts.pivot(index='uid', columns='stress_binary_personal', values='count').fillna(0)
...
label_melted = label_pivot_percent.melt(id_vars='uid', value_vars=['Stress_0', 'Stress_1'],
                                       var_name='Stress_Level', value_name='Percentage')

# Plotting
plt.figure(figsize=(10, 6))
sns.barplot(data=label_melted, x='uid', y='Percentage', hue='Stress_Level', palette='Set2')
...
# Calculate the count of each label per user
label_counts = df_selected.groupby(['uid', 'stress_binary_personal']).size().unstack(fill_value=0)
