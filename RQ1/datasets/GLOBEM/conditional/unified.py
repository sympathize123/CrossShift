...
def normalize_data_per_feature(df: pd.DataFrame) -> pd.DataFrame:
    """Min-max normalize data per feature (column-wise), preserving id/label columns."""
    df_normalized = df.copy()

    numeric_cols = df_normalized.select_dtypes(include=[np.number]).columns
    exclude_cols = [
        # 'stress_binary_personal',
        # 'next_hour_binary_personal',
        'depression_binary', # stress_binary
        'dep', # pss4_EMA
        'pcode',
        'pid',
        'date',
...
def safe_div(num, den):
    """Safe division that returns None when inputs are missing or denominator is zero."""
...
def get_output_paths(dataset_label: str):
    ...
    conditional_dir = os.path.join(rq1_dir, "ConditionalShift", dataset_label)
...
def run_analysis_for_label(df_filtered, data_type, feature_patterns, output_paths, analysis_type, label_status, dataset_label):
    ...
    export_filename = f"{dataset_label}_analysis_data_{data_type}_{analysis_type}_label_{label_status}_{output_paths['timestamp']}.pkl"
    ...
    json_filename = f"{dataset_label}_analysis_data_{data_type}_{analysis_type}_label_{label_status}_{output_paths['timestamp']}.json"
    ...
    print(f"[EXPORT] Analysis data saved for dataset '{dataset_label}', label {label_status}:")
...
def run_analysis_for_data_type(df_hourly_interpretable, data_type, feature_patterns, output_paths, analysis_type, dataset_label, label_col):
    """Run analysis for a specific data type across label values."""
    print(f"\n{'='*80}")
    print(f"ANALYZING DATA TYPE: {data_type.upper()}")
    print(f"{'='*80}")
    
    # Get unique label values
    unique_labels = df_hourly_interpretable[label_col].unique()
    print(f"Found label values: {unique_labels}")
    
    # Run analysis for each label status
    label_results = {}
    for label_status in sorted(unique_labels):
        if label_status in unique_labels:
            ...
            df_filtered = df_hourly_interpretable[df_hourly_interpretable[label_col] == label_status].copy()
            print(f"Filtered data for label {label_status}: {len(df_filtered)} samples")
            
            # Run analysis for this label
            analysis_data = run_analysis_for_label(df_filtered, data_type, feature_patterns, output_paths, analysis_type, label_status, dataset_label)
            if analysis_data:
...
if __name__ == "__main__":
    ...
    print("Conditional Shift Analysis - Unified")
    ...
    for dataset_path in dataset_paths:
        ...
        label_col = None
        for candidate in ('depression_binary',): # 'stress_binary_personal', 'stress_binary'
            if candidate in df_hourly_interpretable.columns:
                label_col = candidate
                break
        if label_col is None:
            raise KeyError(f"Input CSV '{dataset_path.name}' must contain a label column (e.g., 'depression_binary').") # 'stress_binary_personal', 'stress_binary'

        print(f"Original data shape: {df_hourly_interpretable.shape}")
        df_hourly_interpretable = df_hourly_interpretable.dropna(subset=[label_col])
        print(f"After dropping NaN in {label_col}: {df_hourly_interpretable.shape}")
        ...
        for data_type, feature_patterns in DATA_TYPES.items():
            label_results = run_analysis_for_data_type(
                df_hourly_interpretable,
                data_type,
                feature_patterns,
                output_paths,
                analysis_type,
        ...
        for data_type, label_results in all_analysis_data.items():
            ...
            for label_status, analysis_data in label_results.items():
                summary_data['data_type_summaries'][data_type][f'label_{label_status}'] = {
        ...
        comparison_filename = f"{dataset_label}_analysis_comparison_{analysis_type}_{output_paths['timestamp']}.json"
        comparison_path = os.path.join(output_paths['conditional'], comparison_filename)

        label_values_present = sorted(df_hourly_interpretable[label_col].dropna().unique().tolist())
        comparison_data = {
            'metadata': {
                'dataset': dataset_label,
                'analysis_type': analysis_type,
                'timestamp': output_paths['timestamp'],
                'shift_type': 'conditional',
                'labels_analyzed': label_values_present
            },
            'label_comparisons': {}
        }
        ...
        for data_type, label_results in all_analysis_data.items():
            ...
            if 0.0 in label_results and 1.0 in label_results:
                ...
                'statistical_comparison': {
                    ...
                    'permanova_f_ratio': safe_div(label_0_data['statistical_results']['permanova']['F_statistic'], label_1_data['statistical_results']['permanova']['F_statistic']),
                    'permdisp_f_ratio': safe_div(label_0_data['statistical_results']['permdisp']['F_statistic'], label_1_data['statistical_results']['permdisp']['F_statistic'])
                },
                'feature_comparison': {
        ...
        overall_summaries[dataset_label] = {
            'summary': summary_path,
            'comparison': comparison_path,
            'data_types_processed': len(all_analysis_data)
        }

        print(f"\n{'='*80}")
        print(f"CONDITIONAL SHIFT ANALYSIS COMPLETE FOR {dataset_label}")
        print(f"{'='*80}")
        print(f"Analysis type: {analysis_type.upper()}")
