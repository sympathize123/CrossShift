...
# Configuration
# ----------------------------
# Normalization mode: "user" (user-wise) or "feature" (feature-wise)
...
def normalize_data_user_wise(df: pd.DataFrame) -> pd.DataFrame:
    """Standardize data within each pcode group using StandardScaler."""
    df_normalized = df.copy()
    numeric_cols = df_normalized.select_dtypes(include=[np.number]).columns
    exclude_cols = ['stress_binary_personal', 'next_hour_binary_personal', 'pcode']
    numeric_cols = [c for c in numeric_cols if c not in exclude_cols]

    for _, group_data in df_normalized.groupby('pcode'):
        if len(group_data) > 1:
...
if __name__ == "__main__":
    ...
    if NORMALIZATION_MODE == "user":
        ...
        print("Applying feature-wise min-max normalization...")
