...
def get_model_by_name(model_name: str, num_features: int, random_state: int = 42, **kwargs):
    """
    Get a model instance by name with user-configurable hyperparameters.
    
    Args:
        model_name: Name of the model ('xgb', 'lgbm', 'catboost', 'mlp', 
                   'tabtransformer', 'fttransformer', 'node', 'tabresnet')
        num_features: Number of input features
        random_state: Random seed for reproducibility
        **kwargs: Model-specific hyperparameters (e.g., n_estimators, epochs, lr, etc.)
    ...
    Examples:
        # Tree models
        xgb_model = get_model_by_name('xgb', 50, n_estimators=100, max_depth=4)
        lgbm_model = get_model_by_name('lgbm', 50, n_estimators=150, num_leaves=15)
        
        # Deep models
        mlp_model = get_model_by_name('mlp', 50, hidden_layer_sizes=(32, 16), max_iter=100)
        transformer_model = get_model_by_name('tabtransformer', 50, epochs=30, d_model=32)
    """
    model_map = {
    ...
    model_cls = model_map[model_name.lower()]
    
    # Pass appropriate parameters based on model type
    if model_name.lower() in ['xgb', 'lgbm', 'catboost']:
        # Tree models: don't need num_features, use n_jobs=-1 by default
        return model_cls(random_state=random_state, n_jobs=kwargs.get('n_jobs', -1), **kwargs)
    else:
        # Deep models: need num_features, use GPU by default
...
def is_deep_model(model_name: str) -> bool:
    ...
    deep_models = {'mlp', 'tabtransformer', 'fttransformer', 'node', 'tabresnet'}
    return model_name.lower() in deep_models

def is_tree_model(model_name: str) -> bool:
    """Check if a model is a tree-based model (uses parallel computing)."""
    tree_models = {'xgb', 'lgbm', 'catboost'}
...
# Statistical test functions (inherited from utility.py)
def fdr_bh_series(p):
    """Benjamini–Hochberg q-values (Series input)."""
